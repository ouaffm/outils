...
#[test]
fn test_api() {
    let mut tree: AaForest<i64> = AaForest::new(6);
...
#[test]
fn test_bulk_append() {
    let mut tree: AaForest<i64> = AaForest::new(30);
...
fn test_basic_split() {
    ...
    let (left, right) = tree.split(NodeIndex(10), BstDirection::Left);
...
fn test_split_middle() {
    let mut tree: AaForest<i64> = AaForest::new(13);
...
fn test_big_tree_append_and_split() {
    let mut tree: AaForest<i64> = AaForest::new(100);
...
fn test_big_random_tree_append_and_delete() {
    let mut tree: AaForest<i64> = AaForest::new(1000);
...
fn is_binary_search_forest(tree: &mut AaForest<i64>, min: i64, max: i64) -> bool {
...
fn is_binary_search_tree(tree: &AaForest<i64>, node: usize, min: i64, max: i64) -> bool {
    if node == tree.nil {
        return true;
    }
...
fn is_aa_forest(tree: &mut AaForest<i64>) -> bool {
    let iter = tree.arena.iter();
...
fn is_aa_tree(tree: &AaForest<i64>, node: usize) -> bool {
    if node <= tree.sdummy {
        return true;
    }
